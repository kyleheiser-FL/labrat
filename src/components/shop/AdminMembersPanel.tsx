          ...
          {adminMembersList.map(member => (
            <div key={member.id} className="p-5 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 transition-all hover:bg-slate-900/40">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className={`text-[9px] font-extrabold uppercase px-2 py-0.5 rounded tracking-wide ${
                    ...
                    ['kit', 'chinakit', 'chinavial'].includes(member.status) ? 'bg-emerald-500/10 text-emerald-300 border border-emerald-500/20' :
                    member.status === 'blocked'    ? 'bg-red-500/10 text-red-300 border border-red-500/20' :
                                                     'bg-amber-500/10 text-amber-300 border border-amber-500/20'
                  }`}>
                    {['approved', 'kit', 'chinakit', 'chinavial'].includes(member.status) ? 'approved' : member.status}
                  </span>
                </div>
              ...
              <div className="flex flex-col items-end gap-2 self-end md:self-center shrink-0">
                {!['approved', 'kit', 'chinakit', 'chinavial'].includes(member.status) && (
                  <button
                    onClick={() => onSetMemberStatus(member.id, 'approved')}
                    disabled={actionLoading !== null}
                    className="px-3 py-1.5 text-xs font-bold rounded-lg flex items-center gap-1 bg-emerald-500 text-slate-950 shadow shadow-emerald-500/30 cursor-pointer disabled:opacity-50"
                  >
                    <UserCheck className="w-3 h-3" /> Approve Store Access
                  </button>
                )}
