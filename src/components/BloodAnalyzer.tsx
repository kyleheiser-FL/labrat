            ...
            <div className="space-y-3" id="wellness-ledgers-list">
              {metrics.length === 0 ? (
                ...
                metrics.slice().reverse().slice(0, 5).map((m) => (
                  <div key={`metric-row-${m.date}`} className="bg-[#1e293b]/20 border border-slate-800/80 p-3.5 rounded-xl text-xs space-y-1.5">
                    <div className="flex justify-between items-center font-mono font-bold text-slate-300">
                      <div className="flex items-center gap-2">
                        ...
                        {onDeleteMetric && (
                          <button
                            type="button"
                            onClick={() => { triggerHaptic('warning'); onDeleteMetric(m.date); }}
                    ...
                    {m.notes && (
                      <p className="text-[11px] text-slate-400 italic">&ldquo;{m.notes}&rdquo;</p>
                    )}
                  </div>
                ))
              )}
